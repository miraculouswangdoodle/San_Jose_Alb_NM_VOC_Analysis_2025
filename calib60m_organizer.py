"""
CALIB60M File Organizer
=======================
Collects CALIB60M calibration .Chrom files from a nested monthly folder
...
Output:
    A new folder named "CALIB60M files_<root_folder_name>" is created as a
...
def organize_calib60m_files():
    """
    Organizes CALIB60M .Chrom files from a nested folder structure into a
    ...
    Process:
        ...
        3. Finds all .Chrom files containing "CALIB60M" in the filename.
        4. Copies them to a flat folder named "CALIB60M files_<root_folder_name>",
    ...
    # -------------------------------------------------------------------------
    print("=== CALIB60M File Organization Script ===\n")
    ...
    destination_folder = os.path.join(
        Path(root_source).parent, f"CALIB60M files_{root_folder_name}"
    )
    ...
    # -------------------------------------------------------------------------
    print("Scanning for CALIB60M files...")
    ...
    for main_folder_num in range(1, 32):
        ...
        for sub_folder_num in range(1, 32):
            ...
            try:
                # Find all .Chrom files containing "CALIB60M" in this subfolder
                for file in os.listdir(sub_folder):
                    if file.endswith(".Chrom") and "CALIB60M" in file:
    ...
    if not found_files:
        print("No CALIB60M .Chrom files found in the specified folder structure.")
        return

    print(f"\nFound {len(found_files)} CALIB60M file(s) total.\n")
    ...
    print("=" * 50)
    print(f"  Total CALIB60M files found:        {len(found_files)}")
